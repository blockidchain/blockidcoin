...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BID);
    ...
    if (settings.contains("nAnonymizeBlockidCoinAmount") || reset)
        SoftSetArg("-anonymizeblockidcoinamount", settings.value("nAnonymizeBlockidCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in blockidcoin.cpp during shotdown to also remove the window positions
