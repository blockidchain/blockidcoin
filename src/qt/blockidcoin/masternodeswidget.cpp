...
#include "qt/blockidcoin/forms/ui_masternodeswidget.h"
#include "qt/blockidcoin/qtutils.h"
#include "qt/blockidcoin/mnrow.h"
#include "qt/blockidcoin/mninfodialog.h"

#include "qt/blockidcoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(BlockidCoinGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i BID requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 BID required."));
            return;
        }
        showHideOp(true);
