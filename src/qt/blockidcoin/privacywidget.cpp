...
#include "qt/blockidcoin/privacywidget.h"
#include "qt/blockidcoin/forms/ui_privacywidget.h"
#include "qt/blockidcoin/qtutils.h"
#include "guiutil.h"
#include "qt/blockidcoin/denomgenerationdialog.h"
#include "qt/blockidcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(BlockidCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBID anonymizes your BID by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBID or convert back to BID"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BID ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBID");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBID");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBID");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBID");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBID");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBID");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBID");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBID");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zBID Movements"));
    ...
    // Only Convert to BID enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBID->setTitleClassAndText("btn-title-grey", "Total 0 zBID");
    ui->btnTotalzBID->setSubTitleClassAndText("text-subtitle", "Show denominations of zBID owned.");
    ui->btnTotalzBID->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BID outputs to mint into zBID.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zBID");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzBID, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBID");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BID to mint into zBID"));
    }else{
        btnText = tr("Convert back to BID");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBID to convert back into BID"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzBID->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBID->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBID is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBID, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBID").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBID minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zBID converted back to BID"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BID to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBID </b>";
    ...
    ui->btnTotalzBID->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
