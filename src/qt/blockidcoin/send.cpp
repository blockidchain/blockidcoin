...
#include "qt/blockidcoin/send.h"
#include "qt/blockidcoin/forms/ui_send.h"
#include "qt/blockidcoin/addnewcontactdialog.h"
#include "qt/blockidcoin/qtutils.h"
#include "qt/blockidcoin/sendchangeaddressdialog.h"
#include "qt/blockidcoin/optionbutton.h"
#include "qt/blockidcoin/sendconfirmdialog.h"
#include "qt/blockidcoin/myaddressrow.h"
#include "qt/blockidcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(BlockidCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BID)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("BlockidCoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BID");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_BID : AskPassphraseDialog::Context::Send_zBID, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBID is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBID selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zBID transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZBID_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBID require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBID transaction
        ...
        emit message("zBID transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isBID){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zBID in your balance to select."));
        }
    }
}
...
void SendWidget::onBIDSelected(bool _isBID){
    isBID = _isBID;
    setCssProperty(coinIcon, _isBID ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
