...
public:
    ...
    bool WriteZBIDSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBIDSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBIDSeed_deprecated(uint256& seed);
    bool EraseZBIDSeed();
    bool EraseZBIDSeed_deprecated();

    bool WriteZBIDCount(const uint32_t& nCount);
    bool ReadZBIDCount(uint32_t& nCount);
