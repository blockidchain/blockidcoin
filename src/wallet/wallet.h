...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BID at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBID send
enum ZerocoinSpendStatus {
    ZBID_SPEND_OKAY = 0,                            // No error
    ZBID_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBID_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBID_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBID_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBID_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBID_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBID_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBID_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBID_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBID_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBID_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBID_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBID_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBID_SPENT_USED_ZBID = 14,                      // Coin has already been spend
    ZBID_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBID_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBIDOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    void setZWallet(CzBIDWallet* zwallet);
    CzBIDWallet* getZWallet();
    ...
    /// Get 10000 BID output and keys which can be used for the Masternode
    ...
    /** zBID reset */
    boost::signals2::signal<void()> NotifyzBIDReset;
