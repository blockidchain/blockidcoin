...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("blockidcoin-wallet");
...
bool CWalletDB::WriteZBIDSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZBIDSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZBIDSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZBIDSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZBIDSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZBIDSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZBIDCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZBIDCount(uint32_t& nCount)
{
