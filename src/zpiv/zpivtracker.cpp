// Copyright (c) 2018-2020 The BlockidCoin developers
...
CzBIDTracker::CzBIDTracker(std::string strWalletFile)
{
...
CzBIDTracker::~CzBIDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBIDTracker::Init()
{
...
bool CzBIDTracker::Archive(CMintMeta& meta)
{
...
bool CzBIDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBIDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBIDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBIDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBIDTracker::GetSerialHashes()
{
...
CAmount CzBIDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBIDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBIDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBIDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBIDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBIDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBIDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBIDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBIDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBIDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBIDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBIDWallet* zBIDWallet)
{
    bool iszBIDWalletInitialized = (NULL != zBIDWallet);
    CMintMeta meta;
    ...
    if (! iszBIDWalletInitialized)
        zBIDWallet = new CzBIDWallet(strWalletFile);
    meta.isSeedCorrect = zBIDWallet->CheckSeed(dMint);
    if (! iszBIDWalletInitialized)
        delete zBIDWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBIDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBIDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBIDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBIDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBIDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBIDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBIDWallet* zBIDWallet = new CzBIDWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBIDWallet);
        }
        delete zBIDWallet;
