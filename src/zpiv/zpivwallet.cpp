// Copyright (c) 2017-2019 The BlockidCoin developers
...
CzBIDWallet::CzBIDWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBIDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBIDSeed_deprecated()) {
                    LogPrintf("%s: Updated zBID seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBIDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBIDCount(nCountLastUsed);
    else if (!walletdb.ReadZBIDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBIDWallet::Lock()
{
    seedMaster = 0;
}

void CzBIDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBIDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBID(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBIDWallet::LoadMintPoolFromDB()
{
...
void CzBIDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBIDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBIDWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBIDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBID(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBIDWallet::SeedToZBID(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBIDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBIDWallet::GenerateDeterministicZBID(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBIDWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBIDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBIDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
