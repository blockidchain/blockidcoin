...
The build script we'll be using is contained in the BlockidCoin github repository ([contrib/gitian-build.py](https://github.com/blockidcoin-project/blockidcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the BlockidCoin repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/BlockidCoin-Project/BlockidCoin/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, blockidcoin-detached-sigs, and blockidcoin GitHub repos.
