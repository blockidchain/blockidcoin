...
*Setup instructions for a gitian build of BlockidCoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BlockidCoin
...
to the BlockidCoin GitHub release page.
...
- [Building BlockidCoin Core](#building-blockidcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/blockidcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/blockidcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/blockidcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/blockidcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/blockidcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/blockidcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/blockidcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@blockidcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/blockidcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/blockidcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/blockidcoin-osx-*-build.assert
```
